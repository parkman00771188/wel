...
- [`ads.txt`](ads.txt) 는 도메인 루트에서 서빙됩니다. 없으면 AdSense 가 "수익
  위험" 경고를 띄웁니다.
- 콘솔 사이드바의 300×250 자리는 **광고 단위 ID 가 있어야** 채워집니다.
  AdSense → 광고 → 광고 단위 에서 디스플레이 광고를 만들고 그 `data-ad-slot` 을
  [`js/app.js`](js/app.js) 의 `AD_SLOT` 에 넣으면 됩니다. 비어 있는 동안에는
  채워질 수 없는 `<ins>` 를 밀어 넣지 않고 라벨 붙은 placeholder 를 유지합니다.
- 콘솔은 하위 페이지를 iframe 으로 띄우므로 셸과 iframe 양쪽에서 로더가 돕니다.
  자동 광고 배치가 마음에 안 들면 AdSense 대시보드에서 자동 광고를 끄고 위의
  광고 단위만 쓰면 됩니다.
